   ...
   <body>
      <?php
         ...
         if (isset($_GET['valide'])) {
            $reqSql = 'UPDATE commandes SET etat=1 WHERE idCommande = ' . $_SESSION["commandeActuelle"]["idCommande"];
            $req = $connection->prepare($reqSql);
            $req->execute();
            echo '<div class="alert alert-success" role="alert">
               Panier validé avec succès ! Merci pour votre commande.
            </div>
            <div class="text-center">
               <form method="post" action="/index.php">
                  <button type="submit" class="center btn btn-primary"><strong>Retour à l\'accueil</strong></button>
               </form>
            </div>
            ';
         } else {
            $reqSql = 'SELECT * FROM lignesCommandes, commandes WHERE lignesCommandes.idCommande = commandes.idCommande AND commandes.etat = 0';
            $req = $connection->prepare($reqSql);
            $req->execute();
            ...
            echo '
            <div class="text-center">
               <form method="post" action="/panier.php?valide">
                  <button type="submit" class="center btn btn-primary"><strong>Valider le panier</strong></button>
               </form>
            </div>
            <br />';
         }
